...
MONGO_URI        = "mongodb://admin:incident123@localhost:27018/?authSource=admin"
MONGO_DB         = "incidents"
...
def fallback_analysis(service: str, title: str, description: str) -> dict:
    return {
        "summary": f"{service} reported an incident: {title}.",
        "likely_cause": (
            f"The available evidence points to a service or dependency failure. "
            f"Observed signals: {description[:180]}"
        ),
        "recommendations": [
            f"Inspect recent {service} deployments and configuration changes",
            "Check dependency health and connection pool capacity",
            "Review service logs around the incident start time",
        ],
...
def process_incident(raw: dict) -> None:
    ...
    rca_raw = call_ollama(
        prompt,
        fallback=fallback_analysis(service, title, description),
    )
